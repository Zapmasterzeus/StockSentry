...
st.markdown("<h1 style='text-align:center;'>💹 InsightInvest</h1>", unsafe_allow_html=True)
            ...
            try:
                ...
                try:
                    ...
                    try:
                        ...
                        st.error(f"❌ Could not parse JSON output. Please check format.\n\nError: {e}")
                        st.stop()

                # ---------------- HEADER ----------------
                company_name = "Unknown Company"
                try:
                    company_name = data.get("company_overview", "").splitlines()[0].replace("* Company Name:", "").strip()
                except Exception:
                    pass
                ...
                overview_text = data.get("company_overview", "").replace("*", "•").replace("\n", "\n\n")
                st.markdown(f"<div style='background-color:#f8f9fa; padding:12px; border-radius:8px;'>{overview_text}</div>", unsafe_allow_html=True)
                ...
                col3.metric("Debt-to-Equity", round(finance.get("Debt_to_Equity", 0) or 0, 2))
                ...
                with col1:
                    ...
                    sentiment_summary = data.get("sentiment_data", {}).get("summary", {})
                    avg_sent = sentiment_summary.get("average_sentiment")
                    tone = sentiment_summary.get("overall_tone", "Neutral")

                    try:
                        avg_sent_value = float(avg_sent) if avg_sent is not None else 0.0
                ...
                with col2:
                    ...
                    if "image_path" in arima:
                        st.image(arima["image_path"], caption="15-Day Price Forecast", use_container_width=True)
                    else:
                ...
                st.markdown("<center><p style='color:gray;'>📊 Generated by <b>InsightInvest AI Analyst</b></p></center>", unsafe_allow_html=True)
